#!/bin/bash
echo -e "\e[1;32m${bold}Installing needed packages...\e[0m" && sudo apt -y install bash clang libglut-dev libglew-dev pkgconf &> /dev/null
sleep 5s
if [ -d build ]
then
	sleep 1s
...
clang++ ./src/main.cpp -o ./build/ElectroMark -mfp64 -mlong-double-128 $(pkg-config --cflags --libs glut glu glew)
if [ -f ./build/ElectroMark ]
then
